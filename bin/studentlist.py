...
def GetCourses(api_key, page_token, results, logger):
	try:
		r=requests.get('https://classroom.googleapis.com/v1/courses/'+courseId+'/courseWork?pageSize=1000&pageToken'+page_token+'&access_token='+api_key)
		r = json.loads(r.text)

		for coursework in r["courseWork"]:
			result={}

		if "title" in coursework:
			result["title"] = coursework["title"]
		else:
			result["name"] = "(None)"

		if "id" in coursework:
			result["id"] = coursework["id"]
		else:
			result["id"] = "(None)"

		if "maxPoints" in coursework:
			result["maxPoints"] = coursework["maxPoints"]
		else:
...
for result in results:
	try:
		...
		r=requests.get('https://classroom.googleapis.com/v1/courses/'+courseId+'/students?pageSize=10000&access_token='+api_key)
		...
		for student in r["students"]:
			...
			if "profile" in student:
				if "emailAddress" in student["profile"]:
					result["emailAddress"] = student["profile"]["emailAddress"]
			else:
				result["emailAddress"] = "(None)"
		
			if "profile" in student:
				if "fullName" in student["profile"]["name"]:
					result["FullName"] = student["profile"]["name"]["fullName"]
			else:
				result["FullName"] = "(None)"
 		
			if "profile" in student:
                        	if "givenName" in student["profile"]["name"]:
                                	result["FirstName"] = student["profile"]["name"]["givenName"]
                	else:
                        	result["FirstName"] = "(None)"

 			if "profile" in student:
                        	if "familyName" in student["profile"]["name"]:
                                	result["LastName"] = student["profile"]["name"]["familyName"]
                	else:
